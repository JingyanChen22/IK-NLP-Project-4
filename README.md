...
The ```experiments``` folder contains the experiments that replicate the two experiments in the paper Kirov & Cotterell (2018), and also include experiments using new generated English, German and Dutch data. You can run the experiments by starting from ```experiment_phon.ipynb``` for experiments related to phoneme features and ```experiment_orth.ipynb``` for experiments related to orthographic (grapheme) features from the ```experiments``` folder. 
